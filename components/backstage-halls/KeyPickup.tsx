...
export function KeyPickup({ type, position, onPickup }: KeyPickupProps) {
    ...
    const keyWorldPos = useRef(new THREE.Vector3());
    const keyFrameCount = useRef(0);

    // Float and rotate animation, check for player proximity
    useFrame((state) => {
        if (!groupRef.current || collected) return;

        // Floating animation (runs every frame for smoothness)
        const time = state.clock.elapsedTime;
        groupRef.current.position.y = position[1] + Math.sin(time * 2) * 0.2;
        groupRef.current.rotation.y = time * 1.5;

        // Throttle distance check to every 6 frames (auto-pickup still responsive)
        keyFrameCount.current++;
        if (keyFrameCount.current % 6 !== 0) return;
        if (gameState === 'playing' && groupRef.current) {
            groupRef.current.getWorldPosition(keyWorldPos.current);
            const distance = camera.position.distanceTo(keyWorldPos.current);
