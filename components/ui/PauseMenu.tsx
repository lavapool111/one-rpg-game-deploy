...
export function PauseMenu({ defaultOpenInventory = false }: PauseMenuProps) {
    ...
    return (
        <div className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            {/* Menu Container */}
            <div className="bg-slate-900/90 border border-yellow-600/30 p-6 md:p-8 rounded-xl max-w-sm w-full shadow-2xl transform transition-transform scale-100 mx-4">
                <h2 className="text-2xl md:text-3xl font-bold text-center text-yellow-500 mb-6 md:mb-8 tracking-wider uppercase border-b border-yellow-600/20 pb-4">
                    Paused
                </h2>
