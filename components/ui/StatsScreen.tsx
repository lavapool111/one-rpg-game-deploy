...
export function StatsScreen({ onClose }: StatsScreenProps) {
    ...
    const abilityUpgradeStats = getAbilityUpgradeStats();
    ...
    // From ligature (tuba damage) - already applied in ligatureStats
    ...
    let totalCritFactor = critFactor; // Base from store (includes mouthpiece)
    Object.values(equippedEnchantments).forEach((enchant) => {
        if (enchant) {
            const data = getEnchantmentData(enchant.id, enchant.tier);
            if (data.critFactorBonus) totalCritFactor += data.critFactorBonus;
        }
    });
