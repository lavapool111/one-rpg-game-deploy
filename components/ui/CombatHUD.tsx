...
import { getAbilityName, CLASS_INFO } from '@/lib/store/playerStore';
...
export function CombatHUD() {
    ...
    return (
        <div className={`absolute bottom-8 pointer-events-auto z-50 transition-all duration-300 ${isMobile ? 'right-36' : 'right-8'}`}>
            {/* Attack Button */}
            <button
            ...
            >
                ...
                <div className="flex flex-col items-center z-10">
                    <span className="text-2xl mb-1">{CLASS_INFO[playerClass].icon}</span>
                    <span className="text-[10px] font-bold text-white uppercase tracking-wider">
                        {isLongToneActive ? 'ACTIVE' : timeLeft > 0 ? timeLeft.toFixed(1) : getAbilityName(playerClass)}
                    </span>
                </div>
